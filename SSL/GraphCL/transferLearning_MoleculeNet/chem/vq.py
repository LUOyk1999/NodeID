...
class ResidualVectorQuant(nn.Module):
    def __init__(self, dim, codebook_size, num_res_layers=3, **kwargs):
        super().__init__()
        self.num_res_layers = num_res_layers
        self.vq_layers = nn.ModuleList([VectorQuantize(dim=dim, codebook_size=codebook_size, **kwargs) for _ in range(num_res_layers)])
    ...
    def forward(self, x):
        ...
        for layer in self.vq_layers:
            quantized, embed_ind, layer_loss, dist, _codebook = layer(x)
            loss += layer_loss
            dist_list.append(dist)
            _codebook_list.append(_codebook)
            residuals.append(x - quantized)
            quantized_outputs.append(quantized)
            embed_inds.append(embed_ind)  # Store embed_ind for each layer
            x = residuals[-1]  # Update x to be the residual for the next layer

        # Optionally, you can accumulate residuals or combine quantized outputs in a different manner
        final_quantized_output = sum(quantized_outputs)  # This is a simple way to combine outputs

        # The final residual can either be added back or processed differently
        final_output = final_quantized_output

        # Return the final output along with other useful info as needed
        return final_output, embed_inds, loss, dist_list[0], _codebook_list[0]