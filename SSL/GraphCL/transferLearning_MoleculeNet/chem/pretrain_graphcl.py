...
class graphcl(nn.Module):
    ...
    def forward_cl(self, x, edge_index, edge_attr, batch):
        x, total_commit_loss, id_list_concat = self.gnn(x, edge_index, edge_attr)
        x = self.pool(x, batch)
...
def train(args, model, device, dataset, optimizer):
    ...
    for step, batch in enumerate(tqdm(zip(loader1, loader2), desc="Iteration")):
        ...
        x1, total_commit_loss, id_list_concat = model.forward_cl(batch1.x, batch1.edge_index, batch1.edge_attr, batch1.batch)
        x2, total_commit_loss2, id_list_concat2 = model.forward_cl(batch2.x, batch2.edge_index, batch2.edge_attr, batch2.batch)
        loss = model.loss_cl(x1, x2)

        (loss + total_commit_loss2 + total_commit_loss).backward()
        optimizer.step()
