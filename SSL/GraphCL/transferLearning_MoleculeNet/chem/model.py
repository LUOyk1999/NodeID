...
class GNN(torch.nn.Module):
    ...
    """
    def __init__(self, num_layer, emb_dim, JK = "last", drop_ratio = 0, gnn_type = "gin", kmeans=1, num_codes=16):
        ...
        for layer in range(num_layer):
            ...
            if self.kmeans:
                ...
                print("kmeans")
                self.vqs.append(ResidualVectorQuant(dim=emb_dim, codebook_size=num_codes, num_res_layers=3, decay=0.8, commitment_weight=0.25, use_cosine_sim=True, kmeans_init=False))
            else:
                from vqtorch.nn import VectorQuant, ResidualVectorQuant
                print("vq")
                self.vqs.append(ResidualVectorQuant(
                        groups = 3,
                        feature_size=emb_dim,     # feature dimension corresponding to the vectors
    ...
    def forward(self, *argv):
        ...
        for layer in range(self.num_layer):
            ...
            if self.kmeans:
                quantized, _, commit_loss, dist, codebook = self.vqs[layer](x)
                id_list.append(torch.stack(_, dim=1))
                quantized_list.append(quantized)
                total_commit_loss += commit_loss
            else:
                x_, vq_ = self.vqs[layer](x)
        ...
        return node_representation, total_commit_loss, id_list_concat
...
class GNN_graphpred(torch.nn.Module):
    ...
    def __init__(self, num_layer, emb_dim, num_tasks, JK = "last", drop_ratio = 0, graph_pooling = "mean", gnn_type = "gin"):
        ...
        if self.JK == "concat":
            ...
            # self.graph_pred_linear = torch.nn.Linear(15, self.num_tasks)
            self.graph_pred_linear = LogisticRegression(15, self.num_tasks)
    ...
    def forward(self, *argv):
        ...
        node_representation, total_commit_loss, id_list_concat = self.gnn(x, edge_index, edge_attr)

        return self.pool(self.graph_pred_linear(id_list_concat.float()),batch)
