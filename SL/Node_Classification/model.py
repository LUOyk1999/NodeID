...
class GNN(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, local_layers=3,
            in_dropout=0.0, dropout=0.5, heads=1, beta=-1, pre_ln=False, kmeans=1, num_codes=16, gnn='gat'):
        ...
        for _ in range(local_layers):
            ...
            if self.kmeans:
                ...
                print("kmeans")
                self.vqs.append(ResidualVectorQuant(dim=hidden_channels*heads, codebook_size=num_codes, num_res_layers=3, decay=0.8, commitment_weight=0.25, use_cosine_sim=True, kmeans_init=False))
            else:
                from vqtorch.nn import VectorQuant, ResidualVectorQuant
                print("vq")
                self.vqs.append(ResidualVectorQuant(
                        groups = 3,
                        feature_size=hidden_channels*heads,     # feature dimension corresponding to the vectors
