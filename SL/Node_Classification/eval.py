...
def evaluate(model, dataset, split_idx, eval_func, criterion, args, result=None):
    if result is not None:
        ...
        model.eval()
        out, total_commit_loss, id_list_concat, gnn_id = model(dataset.graph['node_feat'], dataset.graph['edge_index'])
    ...
    return train_acc, valid_acc, test_acc, valid_loss, out, id_list_concat
...
def evaluate_cpu(model, dataset, split_idx, eval_func, criterion, args, device, result=None):
    ...
    out, total_commit_loss, id_list_concat, gnn_id = model(x, edge_index)
    id_list_concat = id_list_concat.detach().cpu().numpy()
    train_acc = eval_func(
        dataset.label[split_idx['train']], out[split_idx['train']])
    valid_acc = eval_func(
