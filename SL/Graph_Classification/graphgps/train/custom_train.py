...
def train_epoch(logger, loader, model, optimizer, scheduler, batch_accumulation):
    ...
    for iter, batch in enumerate(loader):
        ...
        (pred, true), total_commit_loss, id_list_concat = model(batch)
        id_list_concat_list.append(id_list_concat)
    ...
    return torch.cat(id_list_concat_list,dim=0), torch.cat(true_list,dim=0)
...
def custom_train(loggers, loaders, model, optimizer, scheduler):
    ...
    for cur_epoch in range(start_epoch, cfg.optim.max_epoch):
        ...
        graph_id, true = train_epoch(loggers[0], loaders[0], model, optimizer, scheduler, cfg.optim.batch_accumulation)
        print(graph_id.shape, true.shape)
        np.savez(f"semantic_ID_{cfg.dataset.name}_train_id", graph_id.detach().cpu().numpy())
        np.savez(f"semantic_ID_{cfg.dataset.name}_train_y", true.detach().cpu().numpy())
        perf[0].append(loggers[0].write_epoch(cur_epoch))

        if is_eval_epoch(cur_epoch):
            for i in range(1, num_splits):
                graph_id, true = eval_epoch(loggers[i], loaders[i], model,
                           split=split_names[i - 1])
                perf[i].append(loggers[i].write_epoch(cur_epoch))
                if(i==1):
                    np.savez(f"semantic_ID_{cfg.dataset.name}_valid_id", graph_id.detach().cpu().numpy())
                    np.savez(f"semantic_ID_{cfg.dataset.name}_valid_y", true.detach().cpu().numpy())
                else:
                    np.savez(f"semantic_ID_{cfg.dataset.name}_test_id", graph_id.detach().cpu().numpy())
                    np.savez(f"semantic_ID_{cfg.dataset.name}_test_y", true.detach().cpu().numpy())
        else:
            for i in range(1, num_splits):
